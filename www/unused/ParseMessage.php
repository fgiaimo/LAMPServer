...
class MessageParser{
    ...
    public function betole($lengthBytes) {
    ...
    // this function compares the length of the message received, and the length that the string is expected to have. 
    public function checkContainer($message) {
        ...
        if (strlen($message) != $protoLength *2 +10) { // strlen(message) returns the length of every char, so 2 chars in a byte. 10 is the number of chars(5bytes) not considered(odbytes, length) in the expected length.              
            ...
            echo "Wrong Length.\n Message: '".$message."' with length ".$strlen ." not equal to expected length: ".$hexlength . "\n";
        ...
        if ($odBytes != $bytesForOD ) {
            echo "odBytes: '". $odBytes."': wrong OD bytes\n";
            return false;
        }
        return true;
     }
